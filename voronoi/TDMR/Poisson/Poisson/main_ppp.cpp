...
const int   NumPoints   = 100;	// 2000 minimal number of points to generate
const bool  Circle      = false;	// 'true' to fill a circle, 'false' to fill a rectangle
//const int   ImageSize   = 1000;	//1024 generate RGB image [ImageSize x ImageSize]
const int   ImageSizeW = 1000;
const int   ImageSizeH = 1000;
const int   k           = 500;		// refer to bridson-siggraph07-poissondisk.pdf for details
...
void LoadDensityMap( const char* FileName )
{
	...
	if ( W != ImageSizeW || H != ImageSizeH )
	{
		std::cout << "ERROR: density map should be " << ImageSizeW << " x " << ImageSizeW << std::endl;		
...
int main( int argc, char** argv )
{
	...
	// prepare BGR image
	//size_t DataSize = 3 * ImageSize * ImageSize; //change ; ImageSize*ImageSize;
	size_t DataSize = 3 * ImageSizeW * ImageSizeH;
	unsigned char* Img = new unsigned char[ DataSize ];
	...
	{
		int x = int( i->x * ImageSizeW );
		int y = int( i->y * ImageSizeH );
		if ( g_DensityMap )
		{
			// dice
			float R = RandomFloat();
			float P = g_DensityMap[ x + y * ImageSizeH ];
			if ( R > P ) continue;
		}
		int Base = 3 * (x + y * ImageSizeH);
		Img[ Base+0 ] = Img[ Base+1 ] = Img[ Base+2 ] = 255;
	}

	SaveBMP( "Poisson.bmp", Img, ImageSizeW, ImageSizeH );
	...
	for (std::vector<sPoint>::iterator p = Points.begin(); p != Points.end(); p++)
	{
