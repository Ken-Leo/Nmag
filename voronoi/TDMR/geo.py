...
class Cylinder:
    ...
    def create_surface(self, points):
        # create line
        line_list = []
        for i in range(len(points) - 1): # connect the dots
            line_list.append(Line(points[i], points[i+1]))
        line_list.append(Line(points[-1], points[0])) # don't forget the start point and end point
        # create line loop
...
def read_points(file_name='Region.txt'):
        region_point_list = []
            ...
            for line in f:
                ...
                for token in line_token:
                    if token == '\n':
                        continue
                    tokens = token.split(',')
                    # append a tuple element to the point_list
